...
empty room seen from above is a smooth floor gradient with no silhouette and no parallax — nothing
for the model to lock onto, and the rotation stops reading. The subject standing on the floor gives
all three, plus the contact point that fixes scale.

**What the orbit is actually for**: the frame you pull out of it shares its camera with the render
you are finishing. An appearance reference shot from a *different* angle drags its own horizon into
the result, and when it shows the same subject it steals the pose outright. Fly from the viewpoint
you have already look-developed to the viewpoint you want to finish, take the last frame, and the
reference becomes geometrically yours — perspective, floor convergence and light pools all belong
to the target camera. The final pass is then only "put this subject into this environment". Drift
accumulates towards the end of the flight, which is exactly where that frame is, so keep the arc
...
python fal_registry.py limits             # refresh fal_schema.json — what the nodes validate against
```

The catalog (1400+ models) and per-model parameter schemas are reachable with a normal
`FAL_KEY` — no admin key needed. `limits` needs no key at all: the schemas are public.

## Cost badge — what FAL actually charged

A badge in the bottom-left corner of ComfyUI: `FAL  $0.312 run · $4.20 today`. It updates
after every FAL call, starts the run at $0 on each queue, and on hover names the last call
and how its cost was worked out.

The figures are what FAL billed, not an estimate. Every result FAL returns carries an
`x-fal-billable-units` header, already counted in the endpoint's priced unit, and FAL's
pricing API (readable with an ordinary key) gives the unit price: units x price is the cost.
`fal_client` drops that header, so `subscribe()` fetches the finished result once more (free)
to read it. Nodes over OpenRouter take the cost the model reports in its own response. A
refusal FAL did not bill (it sends `0` units, as it does for a 422) is not counted; one it
did bill is counted and marked so. Where FAL sends no header the call shows `cost unknown`
rather than a guess.

"Today" lives in `.fal_spend.json` next to the pack (git-ignored), so it survives restarts
and is shared by every ComfyUI that mounts this folder. The day turns over at midnight in
`FAL_COST_TZ` (an IANA zone such as `America/Argentina/Buenos_Aires`), else in the
container's local time — which in Docker is usually UTC. The run total is per ComfyUI
process. Accounting can never fail a job: anything that goes wrong in it degrades to
`cost unknown`.
...
## Before the call, and when it fails

Every node reaches FAL through one function, `subscribe()` in `fal_common.py`.

**Before the call** it checks the arguments against `fal_schema.json` — FAL's own input
schema for every endpoint the pack calls: required fields, allowed enum values, list
bounds. An empty upload is caught too. A refusal here costs nothing and names the field.
The cache is generated by `fal_registry.py limits` and committed; it is never edited by
hand, because a hand-typed bound drifts and starts rejecting calls FAL would take.
`FAL_SKIP_VALIDATION=1` sends anyway — a stale cache must never block a good call.

**When it fails** the error says which kind of failure it is, because each needs a
different reaction:

| Failure | Reads as | What to do |
|---|---|---|
| content filter (NSFW, policy, IP) | `refused by the content filter` | rephrase or swap the reference — the same input refuses again |
| 422 | `FAL rejected the arguments` | compare with `fal_registry.py schema <endpoint>` |
| 429 | `rate limited` | wait, resubmit |
| 401 / 403 | `FAL rejected the key` | check `FAL_KEY` in the container env |
| 5xx | `FAL server error` | their side, resubmit |
| timeout | `timed out … billable either way` | check the FAL dashboard first |
...
fal_cost.py        what FAL billed per call, run and day — behind the cost badge
fal_registry.py    catalog list / search / schema / diff / limits
fal_schema.json    generated by `limits` — the schemas subscribe() validates against
tests/             test_topaz_args.py and test_validation.py (offline, no key),
                   test_folderio_smoke.py and
